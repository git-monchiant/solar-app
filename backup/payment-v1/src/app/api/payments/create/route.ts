...
const leadLocks = new Map<string, Promise<CreateResult>>();

async function handleCreate(leadId: number, force: boolean): Promise<CreateResult> {
  try {
    ...
    for (const row of toCancel.recordset) {
      if (row.kbank_qr_id) {
        try {
          await cancelQR(row.kbank_qr_id);
        } catch (err) {
          console.error(`cancelQR failed for ${row.kbank_qr_id}:`, err);
        }
      }
