...
const h = (await pool.request().query(`SELECT h.id, h.project_id, h.project_name, h.house_number, h.is_om, h.segment,
  i.id inst_id, i.install_date, i.inverter_kw, i.po_number FROM om_houses h LEFT JOIN om_installations i ON i.house_id=h.id WHERE h.house_number IS NOT NULL`)).recordset;
