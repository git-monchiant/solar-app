...
const r = await pool.request().query(`
  SELECT id, status, order_total, order_pct_before, order_installments FROM leads WHERE id = 519
`);
console.log(r.recordset[0]);
const p = await pool.request().query(`
  SELECT id, slip_field, amount, confirmed_at FROM payments WHERE lead_id = 519 ORDER BY step_no
`);
