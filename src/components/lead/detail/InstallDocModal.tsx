...
// In-app preview for the post-install inspection PDF. Same shape as
// WarrantyModal so users have one consistent "preview + save" UX across all
// generated documents.
export default function InstallDocModal({ leadId, docNo, onClose }: Props) {
  const [saving, setSaving] = useState(false);
  const [mounted, setMounted] = useState(false);
  const { me } = useMe();
  const fileLabel = `install-doc_${docNo || leadId}`;
  const pdfUrl = me?.id ? `/api/install-doc/${leadId}?user_id=${me.id}` : `/api/install-doc/${leadId}`;
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] bg-black/70 flex flex-col safe-top" onClick={onClose}>
      <div className="flex items-center justify-between px-4 py-3 shrink-0">
        <div className="text-white text-sm font-semibold">เอกสารตรวจสอบงานติดตั้ง · {docNo || `#${leadId}`}</div>
