"use client";
import { BoltIcon, XIcon, LineIcon } from "@/components/ui/icons";
...
type ScheduledEvent = {
  ...
  status: string;
  zone: string | null;
  // เฉพาะ event_type='followup': log การติดตามที่ลงนัดนี้ไว้
...
const STATUS_LABEL = (j: ScheduledEvent): string => {
  ...
  if (j.event_type === "followup") return "นัดติดตาม";
  return getStatusLabel(j);
};

// Icon picks the appointment kind so colour can be reserved for zone identity.
function EventIcon({ type, channel }: { type: string; channel?: string | null }) {
  if (type === "block") {
    return (
  ...
  if (type === "followup") {
    // ไอคอนตามช่องทางของ log ที่ลงนัดไว้: LINE / เยี่ยมบ้าน / โทร (default)
    if (channel === "line") {
      return <LineIcon className="w-3.5 h-3.5 shrink-0 text-green-600" strokeWidth={2} />;
    }
    if (channel === "visit") {
      return (
...
interface Props {
  ...
  controlledTeam?: "all" | "survey" | "install" | "block" | "followup";
...
export default function EventCalendarList({ monthsBack, monthsForward, days, zoneFilter = "all", showZoneChips = false, toolbarRight, anchor: controlledAnchor, hideNav = false, controlledZone, controlledTeam = "all", onVisibleMonthChange }: Props) {
  ...
  useEffect(() => {
    apiFetch(`/api/surveys/scheduled${controlledTeam === "followup" ? "?include=followup" : ""}`)
  ...
  const { byMonth, byDate } = useMemo(() => {
    ...
    const filtered = controlledTeam === "all"
      ? zoneFiltered
      : controlledTeam === "followup"
      ? zoneFiltered.filter((s) => s.event_type === "followup")
                        ...
                        <div className="space-y-1">
                          {jobs.map((j) => {
                            ...
                            const barColor = isBlock
                              ...
                              : j.event_type === "followup" ? "#3b82f6" : "#8b5cf6";
                            ...
                            const inner = (
                              <>
                                ...
                                />
                                <EventIcon type={j.event_type} channel={j.last_type} />
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="text-xxs text-gray-500 truncate">
                                    ...
                                    {j.event_type === "followup" && (
                                      <>
                                        {j.last_contact_date ? ` · ติดตามเมื่อ ${formatThaiDate(j.last_contact_date)}` : ""}
                                        {j.last_title ? ` · ${j.last_title}` : ""}
                                        {j.last_note ? ` — ${j.last_note}` : ""}
                                      </>
                                    )}
                                  </div>
                                  {j.event_type === "followup" && (
                                    <div className="text-xxs font-semibold text-amber-600">
                                      นัดติดตามครั้งถัดไป {formatThaiDate(j.event_date)}
                                      {String(j.event_date).slice(0, 10) < todayKey && (
                                        <span className="ml-1.5 text-red-600">· เลยกำหนดติดตาม</span>
                                      )}
                                    </div>
                                  )}
                                </div>
