 ...
 *
 * งาน SLA ทุกตัวเริ่มนับจากการกระทำจริง (บันทึกกิจกรรม ยืนยันการชำระเงิน จองสำรวจ
 * อนุมัติใบเสนอราคา) ซึ่ง endpoint พวกนั้นเรียก syncOperationalSlas ให้ทันทีอยู่แล้ว
 * แต่สิ่งที่ไม่มีใครมาเรียกให้คือ "เวลาผ่านไปเฉย ๆ" — นาฬิกาที่เดินอยู่ต้องขยับจาก
 * active ไป warning ไป breached เอง และต้องบันทึกลงตารางพร้อม event log เพื่อให้
 * รายงานย้อนหลังมีข้อมูล งานนี้ไม่มีเหตุการณ์ให้เกาะ จึงต้องมีรอบเวลามาไล่คำนวณ
 *
 * (เดิมคอมเมนต์ตรงนี้ยก Book Survey เป็นตัวอย่างของงานที่ "นับตั้งแต่ Lead เข้ามา"
 *  ซึ่งไม่จริงแล้วตั้งแต่ migration 195 — ตอนนี้มันเริ่มนับเมื่อได้รับค่าสำรวจ)
 *
