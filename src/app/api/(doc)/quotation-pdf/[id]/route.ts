...
export async function GET(
  ...
  const paymentBlockHtml = (joined: boolean) =>
    `<table class="quote-table payment-block${joined ? " joined" : ""}"><tbody>${joined ? '<tr class="payment-spacer"><td style="width:12mm"></td><td></td><td style="width:32.5mm"></td></tr>' : ""}<tr class="payment-shell"><td colspan="2" class="payment-cell"${joined ? "" : ' style="width:auto"'}><div class="payment-title">เงื่อนไขการชำระเงิน</div><table class="payment-table"><tbody>${paymentRows}</tbody></table><div class="payment-bank-row"><div class="bank-copy"><b>ธนาคาร${esc(bankName)}</b><br>ชื่อบัญชี : ${esc(bankAccountName)}<br>เลขที่บัญชี : ${esc(bankAccountNumber)} สาขา ${esc(bankBranch)}<div class="payment-notice"><b>โปรดตรวจสอบก่อนโอนเงิน</b> · ชำระเงินเข้าบัญชีบริษัทที่ SENA SOLAR ENERGY กำหนดเท่านั้น · ห้ามโอนเข้าบัญชีส่วนบุคคลของ Agent พนักงาน หรือบุคคลอื่น · การชำระเงินนอกเหนือจากช่องทางที่บริษัทกำหนด บริษัทฯ ขอสงวนสิทธิ์ไม่รับผิดชอบต่อความเสียหายที่เกิดขึ้น</div></div><div class="qr"><img src="${paymentQrDataUrl}" alt="QR Payment"></div></div></td><td class="payment-side" style="width:32.5mm"></td></tr></tbody></table>`;
