...
export default function LeadDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const CardWrapper = ({ stepIdx, title, icon, children }: { stepIdx: number; title: string; icon: string; children: React.ReactNode }) => {
    ...
    const container =
      state === "active"
        ? "bg-white border border-active/40 shadow-sm shadow-active/10"
        : state === "done"
        ? "bg-white border border-gray-200/70"
        : "bg-gray-50/60 border border-dashed border-gray-200 pointer-events-none";
    ...
    return (
      <div className={`group relative rounded-xl overflow-hidden transition-all ${container}`}>
        <div className="flex items-center gap-3 px-4 py-3">
          <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${iconBox}`}>
            {state === "done" ? (
          ...
          <div className="flex-1 min-w-0">
            <div className={`text-[10px] font-semibold tracking-[0.1em] uppercase ${state === "locked" ? "text-gray-300" : "text-gray-400"}`}>
              Step {stepNum}
            </div>
            <div className={`text-base font-bold leading-tight tracking-tight ${state === "locked" ? "text-gray-400" : "text-gray-900"}`}>
              {title}
            </div>
          </div>

          {state === "active" && (
            <span className="inline-flex items-center gap-1 text-[10px] font-semibold tracking-wider uppercase text-active bg-active-light border border-active/30 px-2 py-1 rounded shrink-0">
          ...
          {state === "done" && (
            <span className="inline-flex items-center gap-0.5 text-[10px] font-semibold tracking-wider uppercase text-emerald-700 bg-emerald-50 border border-emerald-600/15 px-2 py-1 rounded shrink-0">
          ...
          {state === "locked" && (
            <span className="inline-flex items-center text-[10px] font-semibold tracking-wider uppercase text-gray-300 border border-dashed border-gray-200 px-2 py-1 rounded shrink-0">
              Locked
            </span>
          )}
        </div>
        ...
        {state !== "locked" && (
          <div className="px-4 pb-3 pt-2.5 border-t border-gray-100">{children}</div>
        )}
      </div>
    );
  };
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Top bar — fixed h-16 gradient, aligns exactly with sidebar logo */}
      <div className="bg-gradient-to-br from-primary to-primary-dark text-white safe-top">
        <div className="h-16 px-4 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-base font-bold leading-tight truncate">{lead.full_name}</div>
            <div className="text-[11px] text-white/70 leading-tight mt-0.5 flex items-center gap-1.5">
              <span className="font-mono tabular-nums">{lead.phone}</span>
              <span className="text-white/40">·</span>
              <span>{lead.source === "event" ? "Event" : "Walk-in"}</span>
              {isUpgrade && (<><span className="text-white/40">·</span><span>Upgrade</span></>)}
            </div>
          </div>
          {lead.phone && (
            <a href={`tel:${lead.phone}`} className="w-10 h-10 bg-white/15 rounded-full flex items-center justify-center shrink-0 active:bg-white/25 transition-colors">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      {/* Sub-header — meta + tabs on white (below the 64px top bar, same as sidebar content area) */}
      <div className="bg-white border-b border-gray-100">
        {(lead.project_name || lead.house_number || lead.contact_date) && (
          <div className="px-4 pt-2 pb-1 text-[11px] text-gray-500 flex flex-wrap items-baseline gap-x-1.5">
            {lead.project_name && <span className="truncate">{lead.project_name}</span>}
            {lead.project_name && lead.house_number && <span className="text-gray-300">·</span>}
            {lead.house_number && <span className="font-mono tabular-nums">{lead.house_number}</span>}
            {(lead.project_name || lead.house_number) && lead.contact_date && <span className="text-gray-300">·</span>}
            {lead.contact_date && <span>ติดต่อ {formatDate(lead.contact_date)}</span>}
          </div>
        )}
        <div className="flex">
          <button
            onClick={() => setTab("info")}
            className={`flex-1 py-3 text-sm font-semibold border-b-2 transition-colors ${tab === "info" ? "text-active border-active" : "text-gray-500 border-transparent"}`}
          >
            Info
          </button>
          <button
            onClick={() => setTab("log")}
            className={`flex-1 py-3 text-sm font-semibold border-b-2 transition-colors ${tab === "log" ? "text-active border-active" : "text-gray-500 border-transparent"}`}
          >
      ...
      <div className="flex-1 overflow-y-auto pb-20">
        {tab === "info" ? (
          <div className="p-3 space-y-2">
            ...
            {(() => {
              ...
              return (
                <button
                  ...
                  className="w-full text-left rounded-xl bg-white border border-gray-200/70 hover:border-gray-300 transition-colors"
                >
                  <div className="px-4 py-2.5 flex items-center gap-2.5">
                    <div className={`w-7 h-7 rounded-md flex items-center justify-center shrink-0 ${accentColor}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-[10px] font-semibold tracking-[0.1em] uppercase text-gray-400">ข้อมูลการติดต่อล่าสุด</div>
                      {!isEmpty && <div className="text-xs font-semibold text-gray-900">{headerLabel}</div>}
                      {isEmpty && <div className="text-xs text-gray-400">ยังไม่มีการติดต่อ · แตะเพื่อเพิ่ม</div>}
                    </div>
                    <span className="text-gray-400 text-xs shrink-0">+ เพิ่ม</span>
                  </div>
                  {!isEmpty && (
                    <div className="px-4 pb-3 pt-1.5 border-t border-gray-100">
                      <div className="text-xs text-gray-700 leading-snug whitespace-pre-wrap">{bodyText}</div>
                      {createdBy && (
                        <div className="text-[10px] text-gray-400 mt-1">โดย {createdBy}</div>
                      )}
                    </div>
                  )}
                </button>
            ...
            >
              <SurveyStep lead={lead} state={cardState(1)} refresh={refresh} onAddActivity={t => setModalType(t as ActivityType)} />
            </CardWrapper>
            ...
            >
              <QuotationStep lead={lead} state={cardState(2)} refresh={refresh} />
            </CardWrapper>
